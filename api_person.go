...
func (a *PersonAPIService) GeneratePersonProfileExecute(r PersonAPIGeneratePersonProfileRequest) (*os.File, *http.Response, error) {
	...
	if r.projectParticipationIds != nil {
		...
		if reflect.TypeOf(t).Kind() == reflect.Slice {
			s := reflect.ValueOf(t)
			for i := 0; i < s.Len(); i++ {
				parameterAddToHeaderOrQuery(localVarQueryParams, "projectParticipationIds", s.Index(i).Interface(), "multi")
			}
		} else {
			parameterAddToHeaderOrQuery(localVarQueryParams, "projectParticipationIds", t, "multi")
		}
	}
	if r.skillIds != nil {
		t := *r.skillIds
